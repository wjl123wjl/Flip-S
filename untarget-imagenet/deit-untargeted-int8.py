...
from utils.metrics import imagenet_acc
...
def main():
    ...
    for ext_iter in tqdm(range(attackrargs.target_bit+10)):
            ...
            for batch_idx, (aux_inputs, _) in enumerate(aux_loader):
                ...
                aux_targets = clean_model(normalize(aux_inputs)).logits.topk(1).indices.squeeze(dim=-1)
                # compute output
            ...
            try:
                ...
                acc1 = imagenet_acc(torch.nn.Sequential(normalize, model), small_val_loader, device)[0]
                nbit = len(changed_bit)
                print(f'[+] Flip {nbit} bit: {changed_bit}',flush=True)
                if acc1 < 0.2:
                    break
